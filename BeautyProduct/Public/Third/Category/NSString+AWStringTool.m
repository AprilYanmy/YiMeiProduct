...
@implementation NSString (AWStringTool)
...
+(NSString*)getCurrentReadCount{
    ...
    [formatter setDateFormat:@"MMdd HHmm"];
    ...
    NSArray *conutArrs = [currentTimeString componentsSeparatedByString:@" "];
    
    NSString *tmpStr = [NSString stringWithFormat:@"%.0f",[conutArrs[0] integerValue]*13.2+[conutArrs[1] integerValue]*2.41+arc4random()%100*14];
...
+ (NSString *)aw___setNumberEncryption:(NSString *)tell{
    ...
    return  [tell  stringByReplacingOccurrencesOfString:[tell substringWithRange:NSMakeRange(3,4)]withString:@"****"];
...
+(BOOL)validateCustomSever{
     ...
     * 客服电话
     */
    NSString * MOBILE = @"^((13[0-9])|(15[^4,\\D])|(14[0,1-9])|(18[0,1-9])|(17[0,1-9]))\\d{8}$";
    NSString * CM = @"^\\d{3}-?\\d{3}-?\\d{4}|\\d{3}-?\\d{3}-?\\d{4}&";
    NSString * CU = @"^\\d{4}-?\\d{8}|\\d{4}-?\\d{8}&";
    NSString * PHS = @"^\\d{4}-?\\d{7}|\\d{4}-?\\d{7}&";
    NSString * PHS1 = @"^\\d{3}-?\\d{7}|\\d{4}-?\\d{7}&";
    NSString * PHS2 = @"^\\d{3}-?\\d{8}|\\d{4}-?\\d{8}&";
    NSPredicate *regextestmobile = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", MOBILE];
    NSPredicate *regextestCM = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CM];
    NSPredicate *regextestCU = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CU];
    NSPredicate *regextestPHS = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", PHS];
    ...
    if(([regextestmobile evaluateWithObject:self] == YES)
       ...
       || ([regextestPHS1 evaluateWithObject:self] == YES)
...
-(CGFloat)getSpaceLabelHeight:(CGFloat)height withFont:(CGFloat)font withWidth:(CGFloat)width{
    ...
    CGSize size = [self boundingRectWithSize:CGSizeMake(width, SCREEN_HEIGHT) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size;
    return size.height;
}
...
-(CGFloat)getStringWidthFont:(NSInteger)font{
    ...
    CGRect strSize = [self boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:font]} context:nil];
    
    return strSize.size.width;
}

//获取字符size
-(CGSize)gettextSizeWithStringFloat:(float)strfloatnumber andwidth:(float)with
{
    NSDictionary *attribute = @{NSFontAttributeName: [UIFont systemFontOfSize:strfloatnumber]};
    CGSize size = [self boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    
    if (size.width>with) {
        size = [self boundingRectWithSize:CGSizeMake(with, MAXFLOAT) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    }
...
+(NSString*)stringFromDate:(NSDate*)date
{
    ...
    //设置格式：zzz表示时区
    [dateFormatter setDateFormat:@"yyyy-MM-ddzzz"];
    //NSDate转NSString
...
+(NSString *)getUserLongitude{
    return [[NSUserDefaults standardUserDefaults] valueForKey:@"LONGITUDE"];
}

/** 获取当前用户的纬度 */
+(NSString *)getUserLatitude{
    return [[NSUserDefaults standardUserDefaults] valueForKey:@"LATITUDE"];
}

#pragma mark -- 判断输入 是否为数字
...
/*
+ (NSString*)getUserDefaultKeyString{
    
    NSString * useId =  LWNUserDefaultGetString(USERIDKEY);
    
    return useId;
};

+ (NSString *)getBusinessDefaultKeyString{

    NSString * shopId =  LWNUserDefaultGetString(SHOPIDKEY);
    
    return shopId;
}
 */

#pragma mark -- 判断文字是否为空
+ (BOOL)isInputNil:(NSString *)string{
    if ([string isEqualToString:@""]||string == nil||
        [string isKindOfClass:[NSNull class]]||
        [[string stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] length]== 0||string.length == 0 ||[string isEqualToString:@"(null)"]) {
        return YES;
    }
    return NO;
}

#pragma mark -- 判断文字是否包含
+ (BOOL)aw___isRangeStr:(NSString *)string rangeStr:(NSString *)range{
    
    if([string rangeOfString:range].location != NSNotFound){
        return YES;
    }else{
        return NO;
