...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        // 设置引导页上的跳过按钮
        skipButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.8, DDScreenW*0.1, 50, 25)];
        [skipButton setTitle:@"5s" forState:UIControlStateNormal];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.3, DDScreenH*0.9, DDScreenW*0.4, DDScreenH*0.06)];
                [startButton setTitle:@"开始体验" forState:UIControlStateNormal];
                [startButton setTitleColor:ColorMainBG forState:UIControlStateNormal];
                [startButton.titleLabel setFont:[UIFont systemFontOfSize:18]];
                
                startButton.layer.borderColor = ColorMainBG.CGColor;
                startButton.layer.borderWidth = 1;
                startButton.layer.masksToBounds = YES;
                startButton.layer.cornerRadius = DDScreenH*0.06/2;
...
//        [self addSubview:self.imagePageControl];
        ...
        _time = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(timeOutAction) userInfo:nil repeats:YES];
...
- (void)timeOutAction{
    ...
    [skipButton setTitle:[NSString stringWithFormat:@"%lds",(long)_count] forState:(UIControlStateNormal)];
