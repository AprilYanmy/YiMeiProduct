...
- (id)init {
    ...
    if (self) {
        ...
        self.shi = [standard objectForKey:SHI];
    }
    return self;
}
//获取经纬度
    ...
    [geocoder reverseGeocodeLocation:marsLoction completionHandler:^(NSArray *placemarks,NSError *error)
     {
         if (placemarks.count > 0) {
             ...
             self->_lastCity = placemark.administrativeArea;
             [standard setObject:self->_lastCity forKey:CCLastCity];//省市地址
//             NSLog(@"______%@",_lastCity);
             
             
             self->_lastAddress = placemark.name;
             self->_shi = placemark.locality;
//             NSLog(@"______%@",_lastAddress);
         }
         if (self->_cityBlock) {
             self->_cityBlock(self->_lastCity,self->_shi);
             self->_cityBlock = nil;
         }
         if (self->_addressBlock) {
             self->_addressBlock(self->_lastAddress,self->_shi);
