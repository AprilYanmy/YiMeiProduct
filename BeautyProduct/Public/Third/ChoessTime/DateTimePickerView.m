...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.symbolsStr = @"-";
        if (self.pickerViewMode ==DatePickerViewTimeMode) {
            self.symbolsStr = @":";
        }
        ...
        [cancelButton setTitleColor:ColorMainBG forState:UIControlStateNormal];
        ...
        [chooseButton setTitleColor:ColorMainBG forState:UIControlStateNormal];
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component {
    if (self.pickerViewMode == DatePickerViewDateTimeMode) {
        return ([UIScreen mainScreen].bounds.size.width-40)/4;//5
    }else if (self.pickerViewMode == DatePickerViewDateMode){
...
- (void)pickerView:(UIPickerView *)pickerView
    ...
    if (self.pickerViewMode == DatePickerViewDateTimeMode) {
        ...
        _string = [NSString stringWithFormat:@"%ld%@%.2ld%@%.2ld %.2ld",selectedYear,self.symbolsStr,selectedMonth,self.symbolsStr,selectedDay,selectedHour];//:%.2ld  ,selectedMinute
    }else if (self.pickerViewMode == DatePickerViewDateMode){
        switch (component) {
            case 0:
            {
        ...
        _string = [NSString stringWithFormat:@"%ld%@%.2ld%@%.2ld",selectedYear,self.symbolsStr,selectedMonth,self.symbolsStr,selectedDay];
    }else if (self.pickerViewMode == DatePickerViewTimeMode){
        switch (component) {
            case 0:
            {
        ...
        _string =[NSString stringWithFormat:@"%.2ld%@%.2ld",selectedHour,self.symbolsStr,selectedMinute];
    }
...
-(void)configButtonClick
{
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(aw____dateTimePickerView:didClickFinishDateTimePickerView:)]) {
        [self.delegate aw____dateTimePickerView:self didClickFinishDateTimePickerView:_string];
    }
